...
def instanceOfBrowseOrFilterablePages(page):
    ...
    if settings.DEPLOY_ENVIRONMENT == 'build':
        from ask_cfpb.models import AnswerCategoryPage, AnswerResultsPage, AnswerAudiencePage
        pages = (BrowsePage, BrowseFilterablePage, AnswerCategoryPage, AnswerResultsPage, AnswerAudiencePage)
    else:
...
def get_secondary_nav_items(request, current_page):
    ...
    if settings.DEPLOY_ENVIRONMENT == 'build':
        from ask_cfpb.models import AnswerCategoryPage, AnswerResultsPage, AnswerAudiencePage
        if isinstance(page, (AnswerCategoryPage, AnswerResultsPage, AnswerAudiencePage)):
            from ask_cfpb.models import Category
            return [		
                {		
                    'title': cat.name,		
                    'url': '/ask-cfpb/category-' + cat.slug,		
                    'active': False if not isinstance(page, AnswerCategoryPage) else cat.name == page.ask_category.name		
